...
<div class="embed-code-wrap">
	<h3><?php esc_html_e('Benutzerdefinierte Felder einbetten', 'kleinanzeigen'); ?></h3>
	<span class="description"><?php _e( '<b>Einbettungscodes</b> werden in Vorlagen verwendet, um den Wert der benutzerdefinierten Felder des aktuellen Beitrags zurückzugeben. Codes können für einzelne Felder verwendet werden, indem Du die unten stehenden Links zum Einbetten von Code für jedes Feld verwendest.<br />Du kannst auch den gesamten Block benutzerdefinierter Felder für eine Liste mithilfe des Einbettungscodes anzeigen:', 'kleinanzeigen' ); ?></span>
	<br />
	<code><span style="color:red">&lt;?php</span> echo <strong>do_shortcode('[custom_fields_block]')</strong>; <span style="color:red">?&gt;</span></code>
	<br /><br />
	<span class="description"><?php _e( '<b>Shortcodes</b> werden in Beiträgen, Seiten und Widgets verwendet, um den Wert der benutzerdefinierten Felder des aktuellen Posts zurückzugeben. Verwende sie innerhalb der Schleife in Posts und Widgets', 'kleinanzeigen' ); ?></span>
	<br />
	<code><strong>[custom_fields_block]</strong></code>
</div>

<form action="#" method="post" class="ct-form-single-btn">
	<input type="submit" class="button-secondary" name="redirect_add_custom_field" value="<?php esc_html_e('Benutzerdefiniertes Feld hinzufügen', 'kleinanzeigen'); ?>" />
</form>
<table class="widefat">
	<thead>
		<tr>
			<th><?php esc_html_e('Sortierung', 'kleinanzeigen'); ?></th>
			<th><?php esc_html_e('Feldname', 'kleinanzeigen'); ?></th>
			<th><?php esc_html_e('Feld ID', 'kleinanzeigen'); ?></th>
			<th><?php esc_html_e('WP/Plugins', 'kleinanzeigen'); ?></th>
			<th><?php esc_html_e('Feldtyp', 'kleinanzeigen'); ?></th>
	...
	<tbody>
		...
		<tr class="<?php echo ( $class ); ?>">
			<td>
				...
				<?php if($flag): ?>
				<span class="description"><?php esc_html_e('network', 'kleinanzeigen'); ?></span>
				<?php endif; ?>
				<?php if($i != 1 && ! $flag): ?>
				<span class="ct-up"><a href="<?php echo esc_url( self_admin_url( 'admin.php?page=' . $_GET['page'] . "&ct_content_type=custom_field&direction=up&_wpnonce=$nonce&ct_reorder_custom_field=" . $custom_field['field_id'] )); ?>"><img src="<?php echo $this->plugin_url . 'ui-admin/images/up.png'; ?>" alt="<?php echo esc_attr('Nach Oben', 'kleinanzeigen'); ?>" title="<?php echo esc_attr('Nach Oben', 'kleinanzeigen'); ?>"/></a> </span>
				<?php endif; ?>
				<?php if($i != $last && ! $flag): ?>
				<span class="ct-down"><a href="<?php echo esc_url( self_admin_url( 'admin.php?page=' . $_GET['page'] . "&ct_content_type=custom_field&direction=down&_wpnonce=$nonce&ct_reorder_custom_field=" . $custom_field['field_id'] )); ?>"><img src="<?php echo $this->plugin_url . 'ui-admin/images/down.png'; ?>" alt="<?php echo esc_attr('Nach Unten', 'kleinanzeigen'); ?>" title="<?php echo esc_attr('Nach Unten', 'kleinanzeigen'); ?>"/></a></span>
				<?php endif; ?>
			</td>
			<td>
				<strong>
				...
				<div class="row-actions" id="row-actions-<?php echo esc_attr( $custom_field['field_id'] ); ?>" >
					<?php if(! $flag): ?>
					<span class="edit">
						<a title="<?php esc_html_e('Bearbeite dieses benutzerdefinierte Feld', 'kleinanzeigen'); ?>" href="<?php echo esc_url( self_admin_url( 'admin.php?page=' . $_GET['page'] . '&ct_content_type=custom_field&ct_edit_custom_field=' . $custom_field['field_id'] ) ); ?>"><?php esc_html_e( 'Bearbeiten', 'kleinanzeigen' ); ?></a> |
					</span>
					<?php endif; ?>
					<span>
						<a title="<?php esc_html_e('Einbettungscode anzeigen', 'kleinanzeigen'); ?>" href="#" onclick="javascript:content_types.toggle_embed_code('<?php echo esc_js( $custom_field['field_id'] ); ?>'); return false;"><?php esc_html_e('Einbettungscode', 'kleinanzeigen'); ?></a>
					</span>
					...
					<?php if(! $flag): ?>
					<span class="trash">
						| <a class="submitdelete" href="#" onclick="javascript:content_types.toggle_delete('<?php echo esc_js( $custom_field['field_id'] ); ?>'); return false;"><?php esc_html_e( 'Löschen', 'kleinanzeigen' ); ?></a>
					</span>
					<?php endif; ?>
				</div>
				...
				<form action="#" method="post" id="form-<?php echo esc_attr( $custom_field['field_id'] ); ?>" class="del-form">
					...
					<input type="submit" class="button confirm" value="<?php esc_html_e( 'Feld und Werte', 'kleinanzeigen' ); ?>" name="delete_cf_values" />
					<input type="submit" class="button cancel"  value="<?php esc_html_e( 'Abbrechen', 'kleinanzeigen' ); ?>" onClick="content_types.cancel('<?php esc_js( $custom_field['field_id'] ); ?>'); return false;" />
					<input type="submit" class="button confirm" value="<?php esc_html_e( 'Nur Feld', 'kleinanzeigen' ); ?>" name="submit" />
				</form>
			</td>
			<td><?php echo esc_html( $prefix . $custom_field['field_id'] ); ?></td>
			<td><?php echo ( isset( $custom_field['field_wp_allow'] ) && 1 == $custom_field['field_wp_allow'] ) ? __( 'Erlauben', 'kleinanzeigen' ) : __( 'Verweigern', 'kleinanzeigen' ); ?></td>
		...
		<tr id="embed-code-<?php echo esc_attr( $custom_field['field_id'] ); ?>" class="embed-code <?php echo ( $class ); ?>">
			<td colspan="7">
				<div class="embed-code-wrap">
					...
					<br /><br />
					<span class="description"><?php esc_html_e( 'Der Shortcode gibt die Werte des benutzerdefinierten Felds mit dem angegebenen Schlüssel aus dem angegebenen Beitrag zurück. Die Eigenschaft kann "title", "description", "value", "link" oder "image" sein. Die Eigenschaften "Link" und "Bild" können nur für Upload-Feldtypen verwendet werden. Wenn die Eigenschaft nicht verwendet wird, wird "value" zurückgegeben. Verwende sie innerhalb der Schleife in Posts und Widgets', 'kleinanzeigen' ); ?></span>
					<br />
